...
- `services/agents/financeos_agents/llm.py` — swappable LLM provider behind a thin interface (per spec §4/§10). Default is `OfflineNarrator`: deterministic, dependency-free, runs offline; it drafts a controls-analyst-style explanation + recommended disposition from the structured exception. Set `LLM_API_KEY` (+ optional `LLM_MODEL`) to switch to `AnthropicProvider`, which calls the Messages API via stdlib `urllib` (no SDK dependency). Only the exception's discrepancy is sent — never the vendor/customer master (data minimization, SECURITY.md).
- `financeos_agents/reasoning.py` + `GET /api/exceptions/{id}/explain` return `{narrative, suggestedAction, model, grounded}`. The Exception drawer has an **Explain with AI** button that renders the narrative + a model badge. Deterministic rules still own the disposition; the LLM only explains/justifies the gray-area items.