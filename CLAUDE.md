...
- **File ERP adapter + CSV ingestion** — `FileERPConnector`/`FileBankConnector` (`packages/connectors`) read AP invoices, open AR invoices, and deposits from CSVs; selected via `FINANCEOS_ERP=file` + `FINANCEOS_DATA_DIR` (sample data in `infra/data/`). Upload endpoints `POST /api/ingest/{ap|ar-invoices|ar-deposits}` (raw `text/csv` body, no multipart dep) parse a CSV and rebuild the queues via the agents; templates at `GET /api/ingest/template/{kind}`. Exception Queue and Cash Application screens have Import + Template buttons. Shared format in `financeos_connectors/csv_formats.py`; the agents expose data-in variants (`generate_exceptions_from`, `generate_deposits_from`).
