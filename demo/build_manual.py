...
story += [bullets([
    ...
    "<b>4 \u00b7 Supervisor / agent control plane (V3).</b> A deterministic control plane that watches the agents themselves \u2014 tracking each one&rsquo;s override rate, confidence drift, volume, and error rate against a baseline. It acts as a circuit breaker: an agent that degrades or behaves anomalously is auto-demoted to suggest-only and a human is paged, and any action outside an agent&rsquo;s allowed envelope is gated. Crucially the gating logic is rule-based, not another autonomous LLM \u2014 the guardrail can&rsquo;t itself go rogue. This is the governance counterpart to closed-loop learning; full design in <i>V3_TECH_SPEC.md</i>.",
])]
